...
mem_init_mp(void)
{
	...
	size_t i;
	for (i = 0; i < NCPU; i++) {
		uintptr_t vaddr = KSTACKTOP - i * (KSTKSIZE + KSTKGAP);
		boot_map_region(kern_pgdir, vaddr - KSTKSIZE, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W | PTE_P);
	}
}
...
page_init(void)
{
	...
	for (i = npages - 1; i >= 1; i--) {
		if ((i < io_index || i >= end_index) && i != mpentry_index) {
			pages[i].pp_ref = 0;
	...
	if (pgdir) {
		...
		if (!(pde & PTE_P)) {
			if (create) {
				...
				pde = (pde_t) page2pa(page_table_page) | PTE_P;
				pgdir[pgdir_index] = pde;
			} else {
				return NULL;
			}
		}
		pte_t *page_table_entries = (pte_t *)KADDR(PTE_ADDR(pde));
		pte = &page_table_entries[PTX(va)];
	}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	uintptr_t next_free_addr = base;
	if (size > 0) {
		base += size;
	}
	size = ROUNDUP(size, PGSIZE);
	boot_map_region(kern_pgdir, next_free_addr, size, pa, PTE_P | PTE_W | PTE_PCD | PTE_PWT);
	return (void *)next_free_addr;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t addr;
	for (addr = start_addr; addr < end_addr; addr = ROUNDDOWN(addr + PGSIZE, PGSIZE)) {
		if (addr >= ULIM) {
