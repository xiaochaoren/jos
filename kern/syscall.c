...
sys_exofork(void)
{
	...
	struct Env *current_env, *child_env;
	int r;

	if ((r = envid2env(0, &current_env, true)) < 0) {
		return r;
	}

	if ((r = env_alloc(&child_env, current_env->env_id)) < 0) {
		return r;
	}

	child_env->env_status = ENV_NOT_RUNNABLE;
	memcpy(&child_env->env_tf, &current_env->env_tf, sizeof(struct Trapframe));
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	dst_addr = (uintptr_t)dstva;

	if (src_addr >= UTOP || src_addr % PGSIZE != 0 || dst_addr >= UTOP || dst_addr % PGSIZE != 0) {
		return  -E_INVAL;
	}
	...
	if ((r = envid2env(srcenvid, &src_env, true)) < 0) {
		return r;
	}

	if ((r = envid2env(dstenvid, &dst_env, true)) < 0) {
		return r;
	}
	...
	if ((r = page_insert(dst_env->env_pgdir, page, dstva, perm)) < 0) {
		return r;
	}
